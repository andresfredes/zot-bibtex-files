{
	"translatorID": "b0aa2d4e-c173-48f1-8d79-6f1338df1c97",
	"translatorType": 2,
	"label": "BibTeX and Files",
	"creator": "Andres Fredes",
	"target": "bib",
	"minVersion": "6.0.36",
	"maxVersion": "",
	"priority": 100,
...
function doExport() {
	...
	while (item = Zotero.nextItem()) {
		...
		if (item.creators && item.creators.length) {
			...
			if (author) {
				writeField("author", "{" + author.substring(5) + "}", true);
			}
			if (editor) {
				writeField("editor", "{" + editor.substring(5) + "}", true);
			}
			if (translator) {
				writeField("translator",  "{" + translator.substring(5) + "}", true);
			}
			if (collaborator) {
				writeField("collaborator",  "{" + collaborator.substring(5) + "}", true);
			}
		}
		...
		if (Zotero.getOption('exportFileData')) {
			...
			if (attachmentString) {
				writeField("file", attachmentString.substring(1))
			}

		} else {
			// No files exported - attachment files link to existing Zotero file structure
			if (item.attachments) {
